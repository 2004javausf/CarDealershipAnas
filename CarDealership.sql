...
INSERT INTO CARS VALUES(15, 'Jaguar','XF','NightBlack','Available',340000);
...
INSERT INTO OFFERS VALUES(OFFERSEQ.NEXTVAL,'maria98',1,450000,90000,6000,60,'Accepted');
INSERT INTO OFFERS VALUES(OFFERSEQ.NEXTVAL,'matt34',5,500000,120000,20000,19,'Pending');
...
AS
BEGIN
UPDATE OFFERS SET OFFER_STATUS = 'Accepted' WHERE OFFER_ID = OFFERED_ID;
UPDATE CARS SET CAR_STATUS = 'Owned'
WHERE CAR_ID = (SELECT CAR_ID FROM OFFERS WHERE CAR_ID = CAR_ID);
DELETE FROM OFFERS WHERE (OFFER_STATUS = 'Pending' AND CAR_ID = CAR_OFFER_ID);
COMMIT;
END;
/

-- PROCEDURE TO REJECT A CUSTOMER OFFER
CREATE OR REPLACE PROCEDURE REJECTOFFER(OFFERED_ID INT)
AS
BEGIN
DELETE FROM OFFERS WHERE OFFER_ID = OFFERED_ID;
COMMIT;
END;
/
